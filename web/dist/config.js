...
export async function resolveCorpusBase(fetchImpl = globalThis.fetch.bind(globalThis)) {
  ...
  if (override) {
    // Only http(s) bases: the override arrives in a shareable link, and a
    // crafted javascript:/data: base must not become a fetch target or leak
    // into error text as though it were a corpus host.
    let parsed;
    try {
      parsed = new URL(override, globalThis.location?.href ?? "http://localhost/");
    } catch {
      throw new Error(`?corpus= is not a valid URL`);
    }
    if (parsed.protocol !== "https:" && parsed.protocol !== "http:") {
      throw new Error(`?corpus= must be http(s), got ${parsed.protocol}`);
    }
