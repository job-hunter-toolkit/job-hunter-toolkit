...
# Postings column, with the missing marker turned into a genuine break.
postings = "(strcol(2) eq \"?\" ? NaN : real(strcol(2)))"
...
set label 2 "Daily totals from job boards crawled directly. Shaded spans are gaps in the record, not the market." \
...
plot "jobs_record.txt" using 1:@postings with filledcurves y1=0 \
         linecolor rgb '#2a78d6' notitle, \
     "jobs_record.txt" using 1:@postings with lines linestyle 1 notitle
...
plot "jobs_record.txt" using 1:3 with filledcurves y1=0 linecolor rgb '#eb6834' notitle, \
     "jobs_record.txt" using 1:3 with lines linestyle 2 notitle
